...
%
%------------- BEGIN CODE --------------
function main_dynamicReservoir(folderName, fileName)
...
%         centerPosition = 1/2*(numericalInfo.interfaces(4) + numericalInfo.interfaces(3));
        junctionThickness = 10e-9;
        centerPosition = 190e-6;
        
        producePlot(z, constants, xCenter, xFace, dxC, dxF, [1 2 3 4 5 6 7], uniqueSpecies, [numericalInfo.leftElectrodeBC, numericalInfo.rightElectrodeBC], 'concentration', 7);
        drawnow
        ...
        drawnow
%         
        producePlot(z, constants, xCenter, xFace, dxC, dxF, [], uniqueSpecies, [numericalInfo.leftElectrodeBC, numericalInfo.rightElectrodeBC], 'potential', 9);
        drawnow
%         
        ...
        drawnow
%         
        producePlot(z, constants, xCenter, xFace, dxC, dxF, [1 2 3 4 5 6 7], uniqueSpecies, [numericalInfo.leftElectrodeBC, numericalInfo.rightElectrodeBC], 'current', 11)
        drawnow
%         
        ...
        producePlot(z, constants, xCenter, xFace, dxC, dxF, [], uniqueSpecies, [numericalInfo.leftElectrodeBC, numericalInfo.rightElectrodeBC], 'waterEq', 13)
        drawnow
...
% TEMPORARY VARIABLES, TO BE ADDED TO INPUT FILE IF SUCCESSFUL
lResLeft = 6E-5;
lResRight = 6E-5;
originalRightBC = z(end-nSpecies:end-1);
originalLeftBC = z(1:nSpecies);
...
for ii = (ceil(t/dt)+1):totalSteps
    ...
    while (notConverged) || (t ~= targetTime)
        ...
        if negVals ||nanVals || imagVals
            ...
            % Boundary concentration modification based on successful time
            % step, to account for concentration polarization in FBPM mode.
            [leftBCValues, rightBCValues] = computeExternalSolution(z, dxC, dxF, constants, originalLeftBC, originalRightBC, lResLeft, lResRight);
            BCRelax = dtEff;
            z(1:nSpecies) = z(1:nSpecies) + BCRelax * (leftBCValues - z(1:nSpecies));
            z(end-nSpecies:end-1) = z(end-nSpecies:end-1) + BCRelax * (rightBCValues - z(end-nSpecies:end-1));
            
            disp(num2str(max(leftBCValues)))
            disp(num2str(max(rightBCValues)))
