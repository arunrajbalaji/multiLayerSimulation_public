function [z, t] = doTimeStepOutflow(dxC, dxF, nSpecies, z, zPreviousTimeStep, t, dt, ...
...
wienActivityExponent = [-1/2 -1/4 -1/4 0 1/4 -1/2 1/4];
...
delta_1 = dxF(1)/2;
delta_2 = delta_1 + dxF(2);
delta_3 = delta_2 + dxF(3);
gammaDeriv = -(delta_2^2 - delta_1^2)/(delta_2 - delta_1) / ((delta_3^2 - delta_1^2) - (delta_3 - delta_1)*(delta_2^2 - delta_1^2)/(delta_2 - delta_1));
betaDeriv = (1 - (delta_3 - delta_1)*gammaDeriv)/(delta_2-delta_1);
alphaDeriv = -betaDeriv - gammaDeriv;

gammaConc = (-delta_1^2 + (delta_1*(delta_2^2 - delta_1^2))/(delta_2 - delta_1))/(delta_3^2 - delta_1^2 - (delta_3 - delta_1)/(delta_2 - delta_1)*(delta_2^2 - delta_1^2));
betaConc = (-delta_1 - (delta_3 - delta_1)*gammaConc)/(delta_2 - delta_1);
alphaConc = 1 - betaConc - gammaConc;
...
for ii = 1:nIter
    ...
    leftOutflowCondition = 0;
    rightOutflowCondition = 0;
...
%         if leftOutflowCondition
%             matrixDiffusionFluxIncomplete(1) = 0;
%         end
%         if rightOutflowCondition
...
%         end
        ...
        matrixElectromigrationFlux(1) = 1/2*(vale(1,jj)*e/kb/T)*(2*z(2*(nSpecies+1)) - 2*z((nSpecies+1)))./dxF(1);
        matrixElectromigrationFlux(end) = 1/2*(vale(1,jj)*e/kb/T)*(2*z(end) - 2*z(end-(nSpecies+1)))./dxF(end);
%         if leftOutflowCondition
...
%         end
        ...
        matrixElectromigrationPotentialTerm(1) = 2 * vale(1,jj)*e/kb/T*(z(jj) + z((nSpecies+1)+jj))/2./dxF(1);
        matrixElectromigrationPotentialTerm(end) = 2 * vale(1,jj)*e/kb/T*(z(end-(nSpecies+1)) + z(end))/2./dxF(end);
%         if leftOutflowCondition
...
%         RHSOutflowFlux = [leftSpeciesFlux; zeros(length(dxC)-4, 1); 0];
        
        % Determine whether or not outflow condition needs to be applied
        % Note that this has an overall negative compared to the flux as it
        % is defined mathematically, since it has been moved to the right
        % hand side of the equation.
        totalRHSFlux = RHSDiffusionFlux + RHSElectromigrationFlux + RHSStericFlux; 
        
        if totalRHSFlux(1) > 0
            totalRHSFlux(1) = totalRHSFlux(2);
        end

        if totalRHSFlux(end) < 0
            totalRHSFlux(end) = totalRHSFlux(end-1);
        end

        b(jj:(nSpecies+1):end) = timeTermRHS ...
            + outerDerivativeRightFace.*(totalRHSFlux(2:end)) ...
            - outerDerivativeLeftFace.*(totalRHSFlux(1:end-1)) ...
    ...
    dPhiCenterGauss(1) = -(permDphiDx(2) + 2 * permDphiDx(1))./dxC(2);
    dPhiCenterGauss(end) = -(2 * permDphiDx(end) + permDphiDx(end-1))./dxC(end-1);
    ...
    b((nSpecies+1):(nSpecies+1):end) = ...
        [-nA*litersPerCubicMeter * e * poro(2) .* gaussEquationSpeciesSum(2) ...
        - (permDphiDx(2) .* (z(3*(nSpecies+1)) - z(2*(nSpecies+1))) ...
        - 2 * permDphiDx(1) .* (z(2*(nSpecies+1)) - z((nSpecies+1))))...
        ./dxC(2) ...
        - nA*litersPerCubicMeter * e .* backCharge(2); ...
        ...
        -nA*litersPerCubicMeter * e * poro(3:end-2) .* gaussEquationSpeciesSum(3:end-2) ...
        - (permDphiDx(3:end-1) .* (z(4*(nSpecies+1):(nSpecies+1):end-(nSpecies+1)) - z(3*(nSpecies+1):(nSpecies+1):end-2*(nSpecies+1))) ...
        - permDphiDx(2:end-2) .* (z(3*(nSpecies+1):(nSpecies+1):end-2*(nSpecies+1)) - z(2*(nSpecies+1):(nSpecies+1):end-3*(nSpecies+1))))...
        ./dxC(3:end-2) ...
        - nA*litersPerCubicMeter * e .* backCharge(3:end-2); ...
        ...
        -nA*litersPerCubicMeter * e * poro(end-1) .* gaussEquationSpeciesSum(end-1) ...
        - (2*permDphiDx(end) .* (z(end) - z(end-(nSpecies+1))) ...
        - permDphiDx(end-1) .* (z(end-(nSpecies+1)) - z(end-2*(nSpecies+1)))) ...
        ./dxC(end-1) ...
        - nA*litersPerCubicMeter * e .* backCharge(end-1)];
