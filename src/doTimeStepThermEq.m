function [z, t] = doTimeStepThermEq(dxC, dxF, nSpecies, z, zPreviousTimeStep, t, dt, ...
...
gammaE = ones(length(dxC), nSpecies);
wienAlphaBeta = zeros(length(dxC), nSpecies);
...
for ii = 1:nIter
    ...
    % Set activity coefficient values for species affected by second Wien
    % effect, as required for thermodynamic consistency. Currently, we
    % assume that each species
%     if ii == 1
%         for rIndex = 1:size(rxnInputOutputIndices,1)
%             if rxnInputOutputIndices(rIndex, 3) ~= 0
%                 wienExponential = exp(rxnWienCoeff(rIndex)*wienBeta*abs(dPhidXStar));
%                 acti(2:end-1,rxnInputOutputIndices(rIndex,3)) = acti(2:end-1,rxnInputOutputIndices(rIndex,3)).*sqrt(1./wienExponential);
%             end
%             
%             if rxnInputOutputIndices(rIndex, 4) ~= 0
%                 wienExponential = exp(rxnWienCoeff(rIndex)*wienBeta*abs(dPhidXStar));
%                 acti(2:end-1,rxnInputOutputIndices(rIndex,4)) = acti(2:end-1,rxnInputOutputIndices(rIndex,4)).*sqrt(1./wienExponential);
%             end
%         end
%     end
    ...
    for rIndex = 1:size(rxnInputOutputIndices,1)
        if rxnInputOutputIndices(rIndex, 3) ~= 0
            wienExponential = exp(rxnWienCoeff(rIndex)*wienBeta*abs(dPhidXStar));
            gammaE(2:end-1,rxnInputOutputIndices(rIndex,3)) = gammaE(2:end-1,rxnInputOutputIndices(rIndex,3)).*sqrt(1./wienExponential);
            wienAlphaBeta(2:end-1, rxnInputOutputIndices(rIndex,3)) = rxnWienCoeff(rIndex)*wienBeta;
        end

        if rxnInputOutputIndices(rIndex, 4) ~= 0
            wienExponential = exp(rxnWienCoeff(rIndex)*wienBeta*abs(dPhidXStar));
            gammaE(2:end-1,rxnInputOutputIndices(rIndex,4)) = gammaE(2:end-1,rxnInputOutputIndices(rIndex,4)).*sqrt(1./wienExponential);
            wienAlphaBeta(2:end-1, rxnInputOutputIndices(rIndex,4)) = rxnWienCoeff(rIndex)*wienBeta;
        end
    end
...
%     end
    ...
    for jj = 1:nSpecies
        ...
        wienFieldStrength = wienAlphaBeta(2:end-1, jj) .* abs(z(3*(nSpecies+1):(nSpecies+1):end) - z((nSpecies+1):(nSpecies+1):end-2*(nSpecies+1)))./(dxF(2:end) + dxF(1:end-1));
        matrixWienTermDeltaC = (1/2) * (1./dxF(2:end-1)) .* (wienFieldStrength(2:end) - wienFieldStrength(1:end-1));
        
        cInterp = 1/2 * (z((nSpecies+1)+jj:(nSpecies+1):end) + z(jj:(nSpecies+1):end-(nSpecies+1)));
        dxC_double = dxF(1:end-1) + dxF(2:end);
        matrixWienTermDeltaPhi = cInterp ./ dxF;
        ...
        dcCenterSpeciesTransport = timeTerm ...
                    ...
                    + 1/2*[0; -outerDerivativeRightFace(2:end-1).*matrixWienTermDeltaC(2:end) + outerDerivativeLeftFace(2:end-1).*matrixWienTermDeltaC(1:end-1); 0];
        ...
        dcLeftSpeciesTransport = outerDerivativeLeftFace(2:end).*(-matrixDiffusionFluxIncomplete(2:end-1).*acti(2:end-2,jj) + matrixElectromigrationFlux(2:end-1)  - matrixStericFlux(2:end-1)) ...
            + 1/2 * [outerDerivativeLeftFace(2:end-1) .* matrixWienTermDeltaC(1:end-1); 0];
        ...
        dcRightSpeciesTransport = outerDerivativeRightFace(1:end-1).*(-matrixDiffusionFluxIncomplete(2:end-1).*acti(3:end-1,jj) - matrixElectromigrationFlux(2:end-1)  + matrixStericFlux(2:end-1)) ...
            - 1/2 * [0; outerDerivativeRightFace(2:end-1).*matrixWienTermDeltaC(2:end)];
        ...
        dphiCenterSpeciesTransport = outerDerivativeRightFace.*matrixElectromigrationPotentialTerm(2:end) + outerDerivativeLeftFace.*matrixElectromigrationPotentialTerm(1:end-1) ...
            + [0; sign(dPhidXStar(3:end)) .* (outerDerivativeRightFace(2:end-1) .* matrixWienTermDeltaPhi(3:end-1) * 1/2 .* wienAlphaBeta(4:end-1, jj) ./ dxC_double(3:end)) ...
            - sign(dPhidXStar(1:end-2)) .* (outerDerivativeLeftFace(2:end-1) .* matrixWienTermDeltaPhi(2:end-2) * 1/2 .* wienAlphaBeta(2:end-3, jj) ./ dxC_double(1:end-2)); 0];
        
        % dPhi left
        dphiLeftSpeciesTransport = - outerDerivativeLeftFace(2:end) .* matrixElectromigrationPotentialTerm(2:end-1) ...
            + [-sign(dPhidXStar(2:end-1)) .* (outerDerivativeLeftFace(2:end-1) .* matrixWienTermDeltaPhi(2:end-2) * 1/2 .* wienAlphaBeta(3:end-2, jj) ./ dxC_double(2:end-1)) ...
            - sign(dPhidXStar(2:end-1)) .* (outerDerivativeRightFace(2:end-1) .* matrixWienTermDeltaPhi(3:end-1) * 1/2 .* wienAlphaBeta(3:end-2, jj) ./ dxC_double(2:end-1)); 0];
        
        % dPhi right
        dphiRightSpeciesTransport = -outerDerivativeRightFace(1:end-1) .* matrixElectromigrationPotentialTerm(2:end-1) ...
            + [0; sign(dPhidXStar(2:end-1)) .* (outerDerivativeLeftFace(2:end-1) .* matrixWienTermDeltaPhi(2:end-2) * 1/2 .* wienAlphaBeta(3:end-2, jj) ./ dxC_double(2:end-1)) ...
            + sign(dPhidXStar(2:end-1)) .* (outerDerivativeRightFace(2:end-1) .* matrixWienTermDeltaPhi(3:end-1) * 1/2 .* wienAlphaBeta(3:end-2, jj) ./ dxC_double(2:end-1))];
...
%                     disp(['Max Wien exponential:' num2str(max(wienExponential))])
            ...
            if jj == rxnInputOutputIndices(ll, 3)
                ...
                % constant source term
                    ...
                    % Add reaction term to right hand side
                    RHSReactionTerms = RHSReactionTerms + poro(2:end-1) .* rxnRateCoefficients(2:end-1, ll);
                end
            end
        ...
        b(jj:(nSpecies+1):end) = timeTermRHS ...
            ...
            + [0; (outerDerivativeRightFace(2:end-1) .* RHSGammaE(2:end) ...
            - outerDerivativeLeftFace(2:end-1) .* RHSGammaE(1:end-1)); 0];
    ...
    dcCenterGauss = reshape(6.022e26*e*vale(1,:) .* repmat(poro(2:end-1), [1, nSpecies]), [nSpecies*(length(dxC)-2), 1]);
    ...
    b((nSpecies+1):(nSpecies+1):end) = -6.022e26 * e * poro(2:end-1) .* gaussEquationSpeciesSum(2:end-1) ...
