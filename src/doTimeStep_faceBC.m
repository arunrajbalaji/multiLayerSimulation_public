function [z, t] = doTimeStep_faceBC(dxC, dxF, nSpecies, z, zPreviousTimeStep, t, dt, ...
...
for ii = 1:nIter
    ...
    for jj = 1:nSpecies
        ...
        matrixElectromigrationPotentialTerm(end) = 2 * vale(1,jj)*e/kb/T*(z(end-2*(nSpecies+1)+jj) + z(end-(nSpecies+1)+jj))/2./dxF(end);
        if leftElectrodeBC
...
%                     disp(['Max Wien exponential:' num2str(max(wienExponential))])
    ...
    dphiCenterGauss = [-(permDphiDx(2) + 2 * permDphiDx(1))./dxC(2); ...
        -(permDphiDx(3:end-1) + permDphiDx(2:end-2))./dxC(3:end-2); ...
        -(2 * permDphiDx(end) + permDphiDx(end-1))./dxC(end-1)];
    ...
    b((nSpecies+1):(nSpecies+1):end) = ...
        [-nA*litersPerCubicMeter * e * poro(2) .* gaussEquationSpeciesSum(2) ...
         ...
         - permDphiDx(1) .* (2*z(2*(nSpecies+1)) - 2*z((nSpecies+1))))...
         ./dxC(2) ...
         ...
         - (permDphiDx(end) .* (2*z(end) - 2*z(end-(nSpecies+1))) ...
