function [rowInd, colInd, nnz, nEq, valsArrayBlockSizePerSpecies, COO2CSCIndices, A] = makeSparseMatrixIndexVectors(nSpecies, N)
...
nnz = nSpecies*valsArrayBlockSizePerSpecies + (3*N-8) + nSpecies*(N-2);
...
rowInd(nSpecies*(valsArrayBlockSizePerSpecies)+1:end) = ...
    [(nSpecies+1:nSpecies+1:(N-2)*(nSpecies+1))'; ...
    (2*(nSpecies+1):nSpecies+1:(N-2)*(nSpecies+1))'; ...
    (nSpecies+1:nSpecies+1:(N-3)*(nSpecies+1))'; ...
    repmat((nSpecies+1:nSpecies+1:(N-2)*(nSpecies+1))', [nSpecies, 1])];
colInd(nSpecies*(valsArrayBlockSizePerSpecies)+1:end) = ...
    [(nSpecies+1:nSpecies+1:(N-2)*(nSpecies+1))'; ...
    ((nSpecies+1):nSpecies+1:(N-3)*(nSpecies+1))'; ...
    (2*(nSpecies+1):nSpecies+1:(N-2)*(nSpecies+1))'; ...
    reshape((repmat((1:nSpecies)', [1, N-2]) + repmat(1:(nSpecies+1):((nSpecies+1)*(N-2)), [nSpecies, 1]) - 1)', [nSpecies*(N-2), 1])];
...
% Compue conversion vector for going from COO format to CSC format. This is
