function [z, t] = doTimeStep_highOrder_undoThermo(dxC, dxF, nSpecies, z, zPreviousTimeStep, t, dt, ...
...
%     end
    ...
    for rIndex = 1:size(rxnInputOutputIndices,1)
        if rxnInputOutputIndices(rIndex, 3) ~= 0
            wienExponential = exp(rxnWienCoeff(rIndex)*wienBeta*abs(dPhidXStar));
            gammaE(2:end-1,rxnInputOutputIndices(rIndex,3)) = sqrt(1./wienExponential);
%             wienAlphaBeta(2:end-1, rxnInputOutputIndices(rIndex,3)) = rxnWienCoeff(rIndex)*wienBeta;
            wienAlphaBeta(2:end-1, rxnInputOutputIndices(rIndex,3)) = 0;
        end

        if rxnInputOutputIndices(rIndex, 4) ~= 0
            wienExponential = exp(rxnWienCoeff(rIndex)*wienBeta*abs(dPhidXStar));
            gammaE(2:end-1,rxnInputOutputIndices(rIndex,4)) = sqrt(1./wienExponential);
%             wienAlphaBeta(2:end-1, rxnInputOutputIndices(rIndex,4)) = rxnWienCoeff(rIndex)*wienBeta;
            wienAlphaBeta(2:end-1, rxnInputOutputIndices(rIndex,4)) = 0;
        end
    end
...
%     end
    ...
    for jj = 1:nSpecies
        ...
        dphiCenterSpeciesTransport = outerDerivativeRightFace.*matrixElectromigrationPotentialTerm(2:end) + outerDerivativeLeftFace.*matrixElectromigrationPotentialTerm(1:end-1) ...
            + [0; 0; sign(dPhidXStar(4:end-1)) .* (outerDerivativeRightFace(3:end-2) .* matrixWienTermDeltaPhi(4:end-2) * 1/2 .* wienAlphaBeta(5:end-2, jj) ./ dxC_double(4:end-1)) ...
            - sign(dPhidXStar(2:end-3)) .* (outerDerivativeLeftFace(3:end-2) .* matrixWienTermDeltaPhi(3:end-3) * 1/2 .* wienAlphaBeta(3:end-4, jj) ./ dxC_double(2:end-3)); 0; 0];
        
        % dPhi left
        dphiLeftSpeciesTransport = - outerDerivativeLeftFace(2:end) .* matrixElectromigrationPotentialTerm(2:end-1) ...
            + [0; -sign(dPhidXStar(3:end-2)) .* (outerDerivativeLeftFace(3:end-2) .* matrixWienTermDeltaPhi(3:end-3) * 1/2 .* wienAlphaBeta(4:end-3, jj) ./ dxC_double(3:end-2)) ...
            - sign(dPhidXStar(3:end-2)) .* (outerDerivativeRightFace(3:end-2) .* matrixWienTermDeltaPhi(4:end-2) * 1/2 .* wienAlphaBeta(4:end-3, jj) ./ dxC_double(3:end-2)); 0; 0];
        
        % dPhi right
        dphiRightSpeciesTransport = -outerDerivativeRightFace(1:end-1) .* matrixElectromigrationPotentialTerm(2:end-1) ...
            + [0; 0; sign(dPhidXStar(3:end-2)) .* (outerDerivativeLeftFace(3:end-2) .* matrixWienTermDeltaPhi(3:end-3) * 1/2 .* wienAlphaBeta(4:end-3, jj) ./ dxC_double(3:end-2)) ...
            + sign(dPhidXStar(3:end-2)) .* (outerDerivativeRightFace(3:end-2) .* matrixWienTermDeltaPhi(4:end-2) * 1/2 .* wienAlphaBeta(4:end-3, jj) ./ dxC_double(3:end-2)); 0];
        ...
        dPhiRightRightAll(jj:nSpecies:end) = -sign(dPhidXStar(4:end-1)) .* (outerDerivativeRightFace(3:end-2) .* matrixWienTermDeltaPhi(4:end-2) * 1/2 .* wienAlphaBeta(5:end-2, jj) ./ dxC_double(4:end-1));
        dPhiLeftLeftAll(jj:nSpecies:end) = + sign(dPhidXStar(2:end-3)) .* (outerDerivativeLeftFace(3:end-2) .* matrixWienTermDeltaPhi(3:end-3) * 1/2 .* wienAlphaBeta(3:end-4, jj) ./ dxC_double(2:end-3));
