function [z, t] = doTimeStep(dxC, dxF, nSpecies, z, zPreviousTimeStep, t, dt, ...
...
for ii = 1:nIter
    ...
    % phi interpolated to cell edges, size = N-1, useful for reaction term
    phiInterpolated = (z(2*(nSpecies+1):(nSpecies+1):end) + z((nSpecies+1):(nSpecies+1):end-(nSpecies+1)))/2;
    
    % dPhidX compted at cell centers, size = (N-2). Assumes Dirichlet,
    % useful for reaction term (second Wien effect)
    dPhidXStar = (phiInterpolated(2:end) - phiInterpolated(1:end-1))./dxC(2:end-1);
...
%                     disp(['Max Wien exponential:' num2str(max(wienExponential))])
    ...
    vals(nSpecies*(valsArrayBlockSizePerSpecies)+1:end) = ...
        [dphiCenterGauss; ...
        dphiLeftGauss; ...
        dphiRightGauss; ...
        dcCenterGauss];
    ...
    b((nSpecies+1):(nSpecies+1):end) = -6.022e26 * e * poro(2:end-1) .* gaussEquationSpeciesSum(2:end-1) ...
         ...
         ./dxC(2:end-1) ...
         - 6.022e26 * e * poro(2:end-1) .* backCharge(2:end-1);
